...
void DrawSpaceshipBody() // Modified from DrawSpaceshipBody
{
    // Syringe Barrel
    glRotated(LaserAngle, 0, 0, 1);
    glColor3f(0.8, 0.8, 0.8); // Grey color for the barrel
    glPushMatrix();
    glScalef(40, 150, 1); // Adjust size of the barrel
    glutSolidCube(1);
    glPopMatrix();

    // Plunger
    glColor3f(0.5, 0.5, 0.5); // Dark grey color for the plunger
    glPushMatrix();
    glScalef(30, 25, 1); // Adjust size of the plunger
    glTranslated(0, -0.6, 0); // Position the plunger below the barrel
    glutSolidSphere(0.5, 50, 25); // Use sphere to simulate elliptical shape
    glPopMatrix();
    
    glColor3f(1.0, 0.0, 0.0); // Red color for the blood
        glPushMatrix();
        glScalef(30, 70, 1); // Adjust size of the blood
        glTranslated(0, 0.5, 0); // Position the blood above the plunger
        glutSolidCube(1); // Use a cube shape for the blood
        glPopMatrix();


    // Needle (acting as the "gun")
    glColor3f(0.5, 0.5, 0.5); // Red color for the piston
        glPushMatrix();
        glScalef(30, 40, 1); // Adjust size of the piston
        glTranslated(0, -2.37, 0); // Position the piston above the barrel
        glutSolidCube(1); // Use a cube shape for the piston
        glPopMatrix();
}
...
void DrawStone(int StoneIndex)
{
    ...
    case 0:
        ...
        glTranslated(xStone[index1], yStone[index1], 0);
        glRotatef(stoneAngle, 0, 0, 1);
            glTranslated(0, 0, 0);
                    glColor3f(0.8f, 0.8f, 0.0f); // Yellow color for the germ
                    glScalef(50, 50, 1); // Increase the size of the germ
                    glutSolidSphere(1, 9, 50); // Main germ sphere

                    // Draw smaller spheres within the main germ sphere to represent bacteria
                    glColor3f(0.2f, 0.2f, 0.2f); // Dark gray color for bacteria
                    glPushMatrix();
                    glTranslatef(0.3, 0.3, 0); // Translate to a position within the germ
                    glutSolidSphere(0.2, 5, 5); // Draw a small bacteria sphere
                    glPopMatrix();

                    glPushMatrix();
                    glTranslatef(-0.3, -0.3, 0); // Translate to another position within the germ
                    glutSolidSphere(0.2, 5, 5); // Draw another small bacteria sphere
                    glPopMatrix();
        break;
    ...
    case 1:
        ...
        glRotatef(stoneAngle, 0, 0, 1);
            glTranslated(0, 0, 0);
                    glColor3f(0.0f, 0.8f, 0.0f); // Green color for the second type of germ
                    glScalef(50, 50, 1); // Increase the size of the germ
                    glutSolidSphere(1, 9, 50); // Main germ sphere
                    // Draw smaller spheres within the main germ sphere to represent bacteria
                    glColor3f(0.2f, 0.2f, 0.2f); // Dark gray color for bacteria
                    ...
                    glutSolidSphere(0.2, 5, 5); // Draw another small bacteria sphere
                    glPopMatrix();
                    // Add more bacteria dots as needed
                    break;
    ...
    case 2:
        ...
        glRotatef(stoneAngle, 0, 0, 1);
            glTranslated(0, 0, 0);
                    glColor3f(1.0F, 0.0f, 0.0f); // Blue color for the third type of germ
                    glScalef(50, 50, 1); // Increase the size of the germ
                    glutSolidSphere(1, 9, 50); // Main germ sphere
                    // Draw smaller spheres within the main germ sphere to represent bacteria
                    glColor3f(0.2f, 0.2f, 0.2f); // Dark gray color for bacteria
...
void DisplayHealthBar() {
    ...
    snprintf(temp, sizeof(temp), "LIFE = %d", alienLife);
    displayRasterText(800, 600, 0.4, temp);
    snprintf(temp, sizeof(temp), "LEVEL : %d", GameLvl);
...
void startScreenDisplay()
{
    ...
    glEnd();
    glColor3f(0, 0, 0);
    displayRasterText1(-420, -680, 0, "Developed By: Kartik Sharma     Nikhil Bansal     Kushagar Sharma");
